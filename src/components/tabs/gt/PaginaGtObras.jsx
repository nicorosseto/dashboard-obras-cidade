    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-center">
        ...
        <p className="text-sm text-gray-600">
          Buscando a planilha [GT - Obras] mais recente. Isso pode levar
          alguns segundos — trocar de módulo cancela a sincronização.
        </p>
      </div>
    </div>
  )
...
// Pop-up de resultado da sincronização — exige confirmação manual.
...
export default function PaginaGtObras({
  ...
  async function handleAtualizarAgora() {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('sync-gt-obras', {
        signal: controller.signal,
      })
      if (error) throw error
      await onAtualizado?.()
      const totalLinhas = data?.total_linhas
      const comProcesso = data?.com_processo
      const semProcesso = data?.sem_processo
      const linhasDash = data?.linhas_dash
      const partes = []
      if (totalLinhas != null)
        partes.push(`${totalLinhas.toLocaleString('pt-BR')} obras na planilha`)
      if (comProcesso != null)
        partes.push(`${comProcesso.toLocaleString('pt-BR')} com nº de processo`)
      if (semProcesso)
        partes.push(`${semProcesso.toLocaleString('pt-BR')} sem nº de processo`)
      if (linhasDash != null)
        partes.push(`${linhasDash.toLocaleString('pt-BR')} linhas no painel DASH`)
      setResultado({
        titulo: 'Sincronização concluída',
        mensagem: partes.length
          ? partes.join('\n')
          : 'A planilha [GT - Obras] foi sincronizada com sucesso.',
        erro: false,
      })
    } catch (e) {
      // Abort deliberado (usuário trocou de módulo) — não é falha, não mostra
      // popup de erro (mesma ressalva do PaginaMultas.jsx: o functions-js do
      // supabase-js sempre re-embrulha qualquer falha como FunctionsFetchError).
  ...
  return (
    <div className="p-4 space-y-4">
      {basesCarregando && (
        <div className="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
          ...
          <span>
            ...
            carregando. Os números de "processo não encontrado" podem mudar
            até o carregamento terminar.
          </span>
        </div>
      )}
      ...
      {podeAtualizar && (
        <div className="flex justify-end">
          <button
            ...
            data-tour="gt-atualizar"
            className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-sm border transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
      ...
      {carregando && linhas.length === 0 ? (
        <LoadingConteudo mensagem="Carregando GT Obras…" />
      ) : (
        <>
          {abaAtiva === 'geral' && (
