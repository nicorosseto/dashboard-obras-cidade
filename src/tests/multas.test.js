...
describe('buildProcessoSet', () => {
  it('normaliza (normProc) os valores do campo informado', () => {
    const s = buildProcessoSet(
      [{ processo: '00123' }, { processo: 'abc' }],
      'processo'
    )
...
describe('situacaoVinculoDe', () => {
  ...
  it('sem_processo quando não há num_processo_normalizado', () => {
    expect(
      situacaoVinculoDe({ num_processo_normalizado: null }, geoSet, fiscSet)
    ).toBe('sem_processo')
  ...
  it('vinculado_sistemaGeo quando bate com o set de sistemaGeo', () => {
    expect(
      situacaoVinculoDe({ num_processo_normalizado: '123' }, geoSet, fiscSet)
    ).toBe('vinculado_sistemaGeo')
  })

  it('vinculado_fiscalizacao quando bate só com o set de fiscalização', () => {
    expect(
      situacaoVinculoDe({ num_processo_normalizado: '789' }, geoSet, fiscSet)
    ).toBe('vinculado_fiscalizacao')
  })

  it('processo_nao_encontrado quando tem chave mas não bate em nenhum set', () => {
    expect(
      situacaoVinculoDe({ num_processo_normalizado: '999' }, geoSet, fiscSet)
    ).toBe('processo_nao_encontrado')
  })
  ...
  it('sistemaGeo tem prioridade sobre fiscalização quando bate nos dois', () => {
    ...
    expect(
      situacaoVinculoDe({ num_processo_normalizado: '1' }, geo, fisc)
    ).toBe('vinculado_sistemaGeo')
  })
})
...
describe('buildProcessoMap', () => {
  it('mapeia normProc(campo) → linha', () => {
    const m = buildProcessoMap(
      [{ processo: '00123', permissionaria: 'NORCREST' }],
      'processo'
    )
  ...
  it('sem dataCampo, mantém a última linha encontrada para a mesma chave', () => {
    const m = buildProcessoMap(
      [
        { processo: '1', permissionaria: 'A' },
        { processo: '1', permissionaria: 'B' },
      ],
      'processo'
    )
  ...
  it('com dataCampo, mantém a linha de data mais recente (padrão buildVistoriaMap)', () => {
    const linhas = [
      {
        id_origem: '789',
        status_simplificado: 'Em andamento',
        data_inicio: '2024-01-01',
      },
      {
        id_origem: '789',
        status_simplificado: 'Solucionado',
...
describe('cruzarMultas', () => {
  const multas = [
    {
      id: 1,
      num_processo_normalizado: '123',
      permissionaria: 'NORCREST PLANILHA',
    },
  ...
  it('marca _situacao_vinculo em cada linha, preservando os demais campos', () => {
    ...
    expect(rows[0]).toMatchObject({
      id: 1,
      _situacao_vinculo: 'vinculado_sistemaGeo',
    })
    expect(rows[1]).toMatchObject({
      id: 2,
      _situacao_vinculo: 'vinculado_fiscalizacao',
    })
    expect(rows[2]).toMatchObject({
      id: 3,
      _situacao_vinculo: 'processo_nao_encontrado',
    })
...
describe('agruparPorVinculo e resumoVinculo', () => {
  ...
  it('agregaSituacaoVinculo omite baldes vazios', () => {
    ...
    expect(r).toEqual([
      {
        situacao: 'vinculado_sistemaGeo',
        nome: 'Vinculada (Sistema Geo)',
...
describe('valorTotalMultas', () => {
  it('soma o campo valor das linhas', () => {
    expect(
      valorTotalMultas([{ valor: 100 }, { valor: 50.5 }, { valor: null }])
    ).toBe(150.5)
  })
...
describe('todasNorcrest', () => {
  it('verdadeiro só quando toda a lista é NORCREST (via _permissionaria_exibir)', () => {
    expect(
      todasNorcrest([
        { _permissionaria_exibir: 'NORCREST/NCR' },
        { _permissionaria_exibir: 'NORCREST/NCJ' },
      ])
    ).toBe(true)
    expect(
      todasNorcrest([
...
describe('agregaMultasPorUnidadeNorcrest', () => {
  ...
  it('"NORCREST" sem sufixo cai no balde "NORCREST"', () => {
    const r = agregaMultasPorUnidadeNorcrest([
      { _permissionaria_exibir: 'NORCREST' },
    ])
...
describe('agregaMultasPorStatus', () => {
  ...
  it('linhas sem status caem em "Sem status"', () => {
    expect(agregaMultasPorStatus([{ status: null }])).toEqual([
...
describe('aplicarFiltrosMultas', () => {
  const linhas = [
    {
      id: 1,
      permissionaria: 'NORCREST/NCR',
      _permissionaria_exibir: 'NORCREST/NCR',
      status: 'LAVRADO',
      _situacao_vinculo: 'vinculado_sistemaGeo',
      subprefeitura: 'AD',
      data_infracao: '2024-01-10',
    },
    {
      id: 2,
      permissionaria: 'HARGROVE',
      ...
      subprefeitura: 'MP',
      data_infracao: '2024-03-05',
    },
    {
      id: 3,
      permissionaria: 'NORCREST/MLG',
      _permissionaria_exibir: 'NORCREST/MLG',
      status: 'NÃO LAVRADO',
      _situacao_vinculo: 'processo_nao_encontrado',
      subprefeitura: 'AD',
