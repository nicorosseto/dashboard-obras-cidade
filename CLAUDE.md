...
- **Didática:** o usuário está aprendendo — explicar termos, dizer onde clicar, ir
  passo a passo.
- **Redesign visual grande (layout de tela inteira, não um ajuste pontual):**
  oferecer uma **prévia em HTML (Artifact)** antes de implementar no código de
  verdade — deu certo em 16/07/2026 (redesign da Home): a prévia comparou
  lado a lado o layout atual × a proposta, com o conteúdo real, e o usuário
  aprovou com pequenos ajustes antes de eu tocar em `.jsx`. Evita retrabalho
  em decisões de layout que são mais baratas de corrigir num mockup do que
  depois de já estarem no componente React.
