...
- **Didática:** o usuário está aprendendo — explicar termos, dizer onde clicar, ir
  passo a passo.
- **🔍 Verificar e confirmar bug ANTES de dizer "achei" ou corrigir (regra de
  12/08/2026, OBRIGATÓRIA):** ao receber um relato de bug, **confirmar** —
  reproduzir visualmente (screenshot/preview) sempre que possível, ou ler o
  código do ponto EXATO citado e explicar por que ele produz o sintoma
  descrito — que o problema está acontecendo mesmo e **no local exato**
  apontado, antes de aplicar correção ou afirmar "encontrei o erro". **Não
  presumir que uma causa plausível é A causa relatada.** Se não conseguir
  confirmar com segurança, **perguntar ao usuário** em vez de seguir em
  frente. ⚠️ **Lição (12/08/2026):** o usuário relatou quebra visual no
  módulo "Multas"; sem confirmar visualmente, apliquei uma correção
  plausível (mas de outro bug real) em 3 módulos e disse "corrigido" — o
  usuário então avisou que o módulo certo era **Emergências**, não Multas,
  e que o bug relatado **continuava lá**. Causa raiz real (só achada depois,
  investigando com calma): `TituloTela` de Emergências vivia **fora** do
  wrapper flex que contém a sidebar (em `App.jsx`), enquanto nos outros
  módulos ele é filho do MESMO wrapper — por isso só em Emergências o
  título ficava "no meio do L" formado pela sidebar + barra de abas, em vez
  de ao lado da sidebar.
